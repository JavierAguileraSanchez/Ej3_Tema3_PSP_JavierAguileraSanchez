...
class TipoCamisaController(
    ...
    fun list(): List<TipoCamisaResponse> =
        service.list().map { TipoCamisaResponse(it.id!!, it.nombre, it.descripcion, "v4_final_test") } // SEÑAL DE PRUEBA

    @GetMapping("/{id}")
    fun get(@PathVariable id: Long): TipoCamisaResponse =
        service.get(id).let { TipoCamisaResponse(it.id!!, it.nombre, it.descripcion, "v4_final_test") } // SEÑAL DE PRUEBA

    @PostMapping
    fun create(@RequestBody req: TipoCamisaRequest): ResponseEntity<TipoCamisaResponse> {
        val saved = service.create(req)
        val body = TipoCamisaResponse(saved.id!!, saved.nombre, saved.descripcion, "v4_final_test")
    ...
    fun update(@PathVariable id: Long, @RequestBody req: TipoCamisaRequest): TipoCamisaResponse {
        ...
        return TipoCamisaResponse(updated.id!!, updated.nombre, updated.descripcion, "v4_final_test")
    }
